...
\subsection*{Compilazione e distribuzione}

Dalla cartella principale, con Bash e un JDK installati:

\begin{lstlisting}
bash estensioni/gui/build-gui.sh
java -jar distribution/gui/mapGUI.jar
\end{lstlisting}

Lo script compila la base locale \texttt{src} e le classi della GUI escludendo i test;
usa \texttt{-{}-release 8}, oppure \texttt{-source 8 -target 8} sui JDK meno
recenti. La classe principale indicata nel manifest è
\texttt{estensioni.gui.RegressionTreeGUI}. Il JAR non richiede librerie grafiche esterne né il driver
