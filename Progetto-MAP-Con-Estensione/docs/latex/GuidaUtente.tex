...
Il manuale descrive l'avvio del server, l'uso del client da console e
l'estensione GUI: apprendimento, visualizzazione e consultazione delle
predizioni, da database o file locali.

\section*{Prerequisiti}

È necessario Java 8 o successivo. La GUI richiede anche un ambiente desktop
con supporto Swing. Il training da file locale \textbf{non richiede MySQL né
il server MAP}: passare direttamente alla sezione \hyperref[sec:gui]{GUI}.

Per apprendere da database, MySQL deve essere in esecuzione sulla macchina
del server MAP. Alla prima installazione eseguire, con un account
amministrativo MySQL, lo script:
...
Avviare il server prima della CLI o del training GUI da database. Sulla stessa
macchina usare \texttt{localhost}; altrimenti il server deve essere raggiungibile
in rete.

Conservare \path{mysql-connector-java-8.0.17.jar} accanto a \path{server.jar}.
