...
Su Windows aprire la cartella \texttt{distribution} e fare doppio clic su
\textbf{\texttt{avvia-gui.bat}}. In alternativa, dal Prompt dei comandi nella
cartella principale del progetto:
\begin{lstlisting}
distribution\avvia-gui.bat
\end{lstlisting}

Lo script esegue \texttt{gui/mapGUI.jar} e imposta la radice del progetto come
directory di lavoro, anche se richiamato da un'altra cartella. Lasciare aperta
la console associata. Si apre \textbf{Progetto Map 2026}, con origine
\textbf{File locale}, stato \textbf{Pronto} e area centrale
\textbf{In attesa del training}.
...
\begin{enumerate}
  ...
  \item Premere \textbf{Avvia training}. Questa modalità non richiede server né MySQL.
\end{enumerate}

\begin{center}
\includegraphics[width=\textwidth]{immagini/gui-locale.png}
\end{center}
\didascalia{Training locale su prova.dat. L'aspetto dei controlli e il tempo dipendono dal sistema.}

Il risultato comprende \textbf{15 esempi}, \textbf{2 attributi},
\textbf{5 nodi}, \textbf{3 foglie} e \textbf{profondità 2}; si abilita
\textbf{Dettagli}. Per un file spostato usare \textbf{Scegli file},
che inserisce il percorso assoluto.
...
Preparare MySQL e \textbf{avviare prima il server MAP} con lo script
\texttt{avvia-server.bat} su Windows, oppure \texttt{avvia-server.sh} su
Linux/macOS. Avviare quindi la GUI con il suo script, che non accetta argomenti:
host e porta si impostano nella finestra. La GUI comunica con il server MAP,
non direttamente con MySQL.
...
Avviato con gli script, il server salva automaticamente \texttt{provaC.dmp}
in \texttt{distribution/server}, sovrascrivendo l'eventuale archivio omonimo.
La GUI non salva una copia sul computer del client e non apre archivi:
per riutilizzarli senza apprendere di nuovo, usare l'opzione \textbf{2} della CLI.
...
Sono inclusi anche \texttt{provaC.dat}, con un attributo continuo, e
\texttt{servo.dat}, più ampio, nella cartella \path{distribution/src}.
Il training non modifica i file. La GUI locale conserva il risultato
soltanto in memoria e non crea archivi \texttt{.dmp}.

\subsection*{Chiusura}

Chiudere la finestra grafica per terminare la GUI; su Windows premere poi un
tasto nella console dello script. Il risultato locale non viene salvato:
se serve nuovamente, ripetere il training sul file.
Il server MAP resta attivo anche dopo la chiusura della GUI o della CLI:
per arrestarlo, premere \textbf{Ctrl+C} nel suo terminale e confermare
l'eventuale richiesta di terminare il processo batch.
